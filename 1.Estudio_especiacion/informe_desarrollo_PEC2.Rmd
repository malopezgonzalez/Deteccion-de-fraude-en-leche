...
```{r}
p <- ggplot(esp, aes(x = Class, fill = Class )) + 
    geom_bar() + 
    theme(legend.position="none") +
    scale_fill_manual(values=c("#fd7f6f", "#7eb0d5", "#b2e061", "#bd7ebe" )) +
    xlab("Clase") + ylab("Núm. muestras") +
    scale_x_discrete(labels=c("Camello", "Vaca", "Cabra", "Oveja"))
p
```
...
```{r}
library(psych)
resumen <- format(describe(esp, skew = T, IQR = T, fast = F), scientific = F)
save_as_docx(flextable(
    format(
        cbind.data.frame(rownames(resumen),resumen), scientific = T)), 
             path = "resumen_esp.docx")
```


La magnitud de las variables, parece similar con en el resumen de las primeras muestras observadas. Sin embargo, se realizará un escalado para asegurar que todas estas tiene el mismo rango de trabajo y no sesgan los resultados.

# Detección de *outliers*

Se procede a detectar *outliers* mediante la observación del rango intercuartílico. Se considerarán outliers todo aquellos que se encuentren 2 veces por encima del 3er cuartil y lo mismo por debajo del primero [@lantz2015a; @reneshbedre2022]. Se ha establecido el valor de 2 como término medio, ya que habitualmente se emplea 1.5 o 3, valores demasiado amplios y restrictivos respectivamente.
