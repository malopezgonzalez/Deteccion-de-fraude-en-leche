...
params:
  seed: 1992
  path_pc_5: "/Users/miguelangellg92/Library/CloudStorage/OneDrive-UniversitatObertadeCatalunya/Master_bioestadistica_i_bioinformatica/10.TFM/0.Datos/Adulteration.AMXproject/Adulteration 30 seconds 0-5%.data.csv"
  path_pc_10: "/Users/miguelangellg92/Library/CloudStorage/OneDrive-UniversitatObertadeCatalunya/Master_bioestadistica_i_bioinformatica/10.TFM/0.Datos/Adulteration.AMXproject/Adulteration 30 seconds 0-10%.data.csv"
...
```{r}
p <- ggplot(adu, aes(x = Class, fill = Class )) + 
    geom_bar() + 
    theme(legend.position="none") +
    scale_fill_manual(values=c("#fd7f6f", "#7eb0d5")) +
    xlab("Clase") + ylab("Núm. muestras") +
    scale_x_discrete(labels=c("Adulterada", "No adulterada"))
p
```
...
```{r}
library(psych)
resumen <- format(describe(adu, skew = T, IQR = T, fast = F), scientific = F)
save_as_docx(flextable(
    format(
        cbind.data.frame(rownames(resumen),resumen), scientific = T)), 
             path = "resumen_adu.docx")
```
...
Se procede a detectar *outliers* mediante la observación del rango intercuartílico. Se considerarán outliers todo aquellos que se encuentren 3 veces por encima del 3er cuartil y lo mismo por debajo del primero [@lantz2015a; @reneshbedre2022].
