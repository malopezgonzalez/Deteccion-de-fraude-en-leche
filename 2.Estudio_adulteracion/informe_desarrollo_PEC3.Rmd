...
La magnitud de las variables parece similar al igual que su rango visto en el resumen de las primeras muestras observadas. Sin embargo, se realizará un escalado para asegurar que todas estas tienen el mismo rango de trabajo y no sesgar los resultados.

```{r}
adu_norm <- mvn(adu[,-1], univariateTest = "SW")
head(adu_norm$univariateNormality, n = 10)
table(adu_norm$univariateNormality$Normality)
```

Se comprueba la normalidad de los datos mediante el test de Shapiro-Wilk, siendo estas primeras variables no normales. De todas formas, no es un dato relevante para los métodos que se quieren aplicar.
